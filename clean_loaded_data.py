...
# Set True -> use LLM (ai_query) to clean job descriptions (removes boilerplate,
# URLs, nav artifacts, non-English text). Adds latency (~2-5 sec/row).
# Set False -> skip LLM cleaning (faster, use for testing or cost savings)
...
GOOGLE_COUNTRY_PATTERNS = [
    ...
    (r"Istanbul.*Turkiye$|,\s*Turkiye$","Turkey"),
...
# ─────────────────────────────────────────────────────────────
# LLM JOB DESCRIPTION CLEANING
# Uses ai_query() with Llama 3.3 70B to remove boilerplate,
# URLs, nav artifacts, and non-English text from descriptions.
# Controlled by ENABLE_LLM_CLEANING flag.
# ─────────────────────────────────────────────────────────────

LLM_CLEANING_PROMPT = (
    "Clean the following job description. Rules:\\n"
    "1. Remove all URLs and hyperlinks\\n"
    "2. Remove boilerplate EEO/accommodation/disability statements\\n"
    "3. Remove navigation artifacts (e.g. 'Explore open roles on our careers page', "
    "'info_outline', 'Apply to this job on...')\\n"
    "4. If the description is bilingual (e.g. French + English), keep ONLY the English portion\\n"
    "5. Remove leading/trailing whitespace and collapse excessive newlines into single newlines\\n"
    "6. Keep ALL substantive job content: role description, responsibilities, qualifications, team info\\n"
    "7. Return ONLY the cleaned text with no commentary or explanation\\n\\n"
    "Job Description:\\n"
)


def clean_job_descriptions(df):
    """Use LLM (ai_query) to clean job_description column.

    Removes boilerplate, URLs, navigation artifacts, and non-English text.
    Only processes rows where job_description is not null.
    """
    return df.withColumn(
        ...
        F.when(
            ...
            F.expr(
                ...
                "  CONCAT("
                f"    '{LLM_CLEANING_PROMPT}',"
                "    job_description"
                "  ),"
                "  modelParameters => named_struct('max_tokens', 4096, 'temperature', 0.0)"
                ")"
            )
        ).otherwise(F.col("job_description"))
    )
...
def process_incrementally(wm, availability, orgs_ready):
    ...
    for org in orgs_ready:
        ...
        try:
            ...
            # Stage 2.5: LLM job description cleaning
            if ENABLE_LLM_CLEANING:
                df_soft = clean_job_descriptions(df_soft)
                print(f"  LLM job description cleaning applied")
